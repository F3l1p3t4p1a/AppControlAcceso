...
export default function RegistroVehiculoSalida({navigation}) {
  ...
  const sendData = async () => {
    ...
    const hora_salida = `${date.getHours()}:${minutos}`;
    ...
    try {
      const data = await fetch(`https://api/api_control_acceso/SalidaVehiculo.php`, {
        method: 'POST',
        headers: {
      ...
      if (dataJson.status == 'ok') {
        Alert.alert('SALIDA ENVIADA', 'La salida de la tripulacion se guardo exitosamente', [
          {
            text: 'ok',
            onPress: () => {
              setPatente('');
      ...
      if(dataJson.status == 'falla'){
        Alert.alert('Error', 'La patente no cuenta con entrada por lo que no puede salir.', [
          {text: 'ok'},
        ])
      }
    } catch (error) {
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <View style={styles.wrapperGeneral}>
        <View style={styles.wrapperPatente}>
          <Text style={styles.title}>Registrar tripulación Salida</Text>
          <TextInput
            placeholder="PATENTE"
